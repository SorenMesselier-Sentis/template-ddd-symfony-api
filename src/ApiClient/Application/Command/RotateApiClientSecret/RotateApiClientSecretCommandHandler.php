...
namespace App\ApiClient\Application\Command\RotateApiClientSecret;
...
final class RotateApiClientSecretCommandHandler
{
     ...
     */
    public function __invoke(RotateApiClientSecretCommand $command): array
    {
        ...
        $entity->rotateSecret(HashedClientSecret::fromPlainSecret($plainSecret));

        $this->repository->save($entity);
        $this->tokenRepository->revokeAllForClient($command->id);
        $this->eventBus->publish(...$entity->pullDomainEvents());

        $this->logger->info('ApiClient secret rotated', ['id' => $command->id]);
