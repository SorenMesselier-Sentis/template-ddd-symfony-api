...
final class SendEmailVerificationOnEmailVerificationRequested
{
    ...
    public function __invoke(EmailVerificationRequested $event): void
    {
        ...
        try {
            $verificationUrl = rtrim($this->frontendUrl, '/').'/verify-email?token='.urlencode($event->token);

            $content = $this->emailTemplateRenderer->render(
                UserEmailTemplate::EMAIL_VERIFICATION,
                [
