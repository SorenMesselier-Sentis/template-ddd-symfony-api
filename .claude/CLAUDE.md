...
A Symfony 8 / PHP 8.4 REST API template structured around Domain-Driven Design with bounded contexts. `User` and `Document` are the reference contexts; forks scaffold new ones with the maker commands. Base API path is `/api/v1`.
...
- **Soft delete everywhere**: entities carry a `status` enum (`active`, `inactive`, `deleted`); repositories exclude deleted rows rather than issuing DELETEs.
