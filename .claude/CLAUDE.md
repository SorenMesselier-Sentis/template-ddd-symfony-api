...
- **GDPR retention/anonymization** (optional per BC, wired for `User`): `Shared\Domain\Privacy\PersonalDataAnonymizerInterface` (`key()`, `anonymizeExpired(\DateTimeImmutable $before): int`) is the automated counterpart to the exporter above — same auto-tag pattern (`app.gdpr_data_anonymizer`), collected by the daily `CleanupExpiredPersonalDataHandler` scheduled task, which scrubs (never hard-deletes) records soft-deleted past `GDPR_RETENTION_DAYS` (default 30). See `User\Application\Privacy\UserPersonalDataAnonymizer` / `User::anonymize()`. A BC opts in by implementing the interface; nothing else needs wiring.
- **Consent** (`User` BC): `Consent` entity + `ConsentType` enum (`terms_of_service`/`privacy_policy`/`marketing_emails`) back `POST`/`GET /users/me/consents` and `DELETE /users/me/consents/{type}` — one row per `(user, type)`, upserted, recorded in the audit trail, included in the GDPR export. Current document versions a client compares against come from `User\Infrastructure\Legal\LegalDocumentVersion`, backed by the placeholder `docs/legal/*.md` (**not** real legal text — replace before production use) and exposed publicly via `GET /legal/documents`.
