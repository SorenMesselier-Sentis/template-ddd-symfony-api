...
- **Error tracking (Sentry, optional)** is wired as Monolog handlers at `error` level (`config/packages/monolog.yaml`, prod only), not the `sentry/sentry-symfony` bundle's own automatic exception listener (`register_error_listener: false` in `sentry.yaml`) — that would report every throwable indiscriminately, including expected `4xx` domain/validation/auth responses. A no-op until `SENTRY_DSN` is set. Consequence: always log a caught exception as `$logger->error($message, ['exception' => $exception])` — the real `\Throwable` object, never `$exception->getMessage()`/`$exception::class` — or Sentry (and structured JSON logs generally) loses the stack trace and groups by message text instead of exception type.
- **Real-time push (Mercure)**: the Mercure hub is embedded directly in the `php` container's FrankenPHP/Caddy process (`docker/frankenphp/Caddyfile`'s `mercure` directive) — no separate container, and `/.well-known/mercure` requests never reach the PHP/Symfony kernel. Any BC publishes via the port `Shared\Domain\RealTime\RealtimePublisherInterface` (`publish(string $topic, array $data, bool $private = true)`), backed by `MercureRealtimePublisher`. Topics are private per-user by default (`/users/{id}/...`); a frontend calls `GET /users/me/realtime-token` first (`User\Infrastructure\Http\Controller\GetRealtimeSubscriberTokenController`) to get a `mercureAuthorization` cookie scoped to its own topics, then opens an `EventSource` to the hub. Wired as a working example on the `IN_APP` notification channel (`InAppChannelNotificationHandler`) — every in-app notification is pushed live, not just logged. `POST /users/me/realtime-test` (`SendTestRealtimeNotificationCommand`/`Controller`) is a minimal end-to-end demo of that path — dispatch it after subscribing to see a live push arrive.
...
- **GDPR right to erasure is mandatory for personal data**: the same BCs **must** also implement `Shared\Domain\Privacy\PersonalDataEraserInterface` (`key()`, `erase(string $subjectId): void`), auto-tagged `app.gdpr_data_eraser` the same way, invoked by both self-service `DELETE /users/me` and admin `DELETE /users/{id}`. Unlike the exporter (read-only), each implementation owns its own load/mutate/save/publish — no generic coordinator. Soft-deleted rows are the norm elsewhere, but an eraser that only flips a `status` flag is not erasure: `User\Application\Privacy\UserPersonalDataEraser` anonymizes name/email/password and revokes every refresh/password-reset/email-verification token; `Document\Application\Privacy\DocumentPersonalDataEraser` purges the S3 object *and* hard-deletes the row (`DocumentRepositoryInterface::delete()`) — original filenames are themselves often identifying, so a soft-deleted row would defeat the purpose. Hard-deleting is the one place this is the right call; don't reach for it outside an eraser.
...
- **Holds personal data about a user?** → implement `PersonalDataExporterInterface` **and** `PersonalDataEraserInterface` (both mandatory, see Architecture above).
