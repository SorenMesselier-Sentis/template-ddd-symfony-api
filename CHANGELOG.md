...
### Added
- A table of contents at the top of `README.md`, and the "Getting started → Installation" section rewritten as a numbered walkthrough (clone/configure → `make init` → verify via `/health/live` and a seeded-user login → stop/restart) instead of a bare command block — it was previously easy to miss 350+ lines into the file with no way to jump to it. `make init` now also runs the new `make jwt-keys` (see below) so a first-time `make init` leaves auth actually usable, instead of requiring the separate "Reproduce the CI pipeline locally" steps to discover JWT keys were needed at all.
- `make jwt-keys`: generates the local JWT keypair (idempotent — skips if `config/jwt/private.pem` already exists), replacing the raw `docker compose ... openssl` snippet previously only documented under "Reproduce the CI pipeline locally".
- Optional random-bulk fixture layer via a new `FixtureFaker` (thin `fakerphp/faker` factory in `Shared/Infrastructure/Fixture/`): `UserFixture`, `DocumentFixture`, `ProjectFixture` and `TaskFixture` now each accept a `$randomCount` wired to its own `FIXTURES_RANDOM_<NAME>_COUNT` env var and append that many extra, unreferenced rows on top of the named John/Jane/Bob dataset, purely to populate the database with a realistic volume for browsing/pagination testing. Random documents/projects/tasks are always owned by one of the three named users, never a made-up id. Defaults to 0 (off) unless set; `.env` turns it on for dev (25/40/10/60), `.env.test` forces all four back to 0 so `HttpTestCase` (which reloads every fixture before each test) stays fast and deterministic. See README "Fixtures and test data".
