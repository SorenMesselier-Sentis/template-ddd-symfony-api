...
use App\Document\Application\Query\GetDocuments\GetDocumentsCursorQueryHandler;
...
final class GetDocumentsCursorQueryHandlerTest extends UnitTestCase
{
    public function testItReturnsCursorPageForAuthenticatedOwner(): void
    {
        ...
        $repository->expects($this->once())
            ->method('findByOwnerIdAndFiltersCursor')
            ->with($ownerId, $filters, $cursorPagination)
            ->willReturn(new CursorPage([$document], 'next-token'));

        $ownerContext = $this->createStub(OwnerContextInterface::class);
        $ownerContext->method('ownerId')->willReturn($ownerId);

        $response = (new GetDocumentsCursorQueryHandler($repository, $ownerContext))->__invoke(
            new GetDocumentsCursorQuery($filters, $cursorPagination),
        );
